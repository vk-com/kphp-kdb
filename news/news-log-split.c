...
char *progname = "news-log-split", *username, *targ_fname;
int verbosity = 0;
int now;
int targ_fd;
int targ_existed;
...
static int get_logrec_size (int type, void *ptr, int size) {
  ...
  case LEV_START:
    ...
    s = 24 + ((E->b + 3) & -4);
    if (size < s) { return -2; }
    if (E->a != NEWS_SCHEMA_USER_V1 && E->a != NEWS_SCHEMA_GROUP_V1 && E->a != NEWS_SCHEMA_COMMENT_V1 && E->a != NEWS_SCHEMA_RECOMMEND_V1) {
      fprintf (stderr, "fatal: NEWS_SCHEMA expected\n");
      return -2;
    }
    if (split_mod) {
  ...
  case LEV_NEWS_USERDEL:
    return sizeof (struct lev_userdel);
  case LEV_NEWS_PRIVACY:
    return sizeof (struct lev_privacy);
  case LEV_NEWS_ITEM+1 ... LEV_NEWS_ITEM+20:
    return sizeof (struct lev_news_item);
  case LEV_NEWS_COMMENT+20 ... LEV_NEWS_COMMENT+24:
    return sizeof (struct lev_news_comment);
  case LEV_NEWS_PLACEDEL+20 ... LEV_NEWS_PLACEDEL+24:
    return sizeof (struct lev_news_place_delete);
  case LEV_NEWS_HIDEITEM+20 ... LEV_NEWS_HIDEITEM+24:
    return sizeof (struct lev_news_comment_hide);
...
static int want_write (int type, void *rec) {
  ...
  case LEV_NEWS_ITEM+1 ... LEV_NEWS_ITEM+20:
    ...
    break;
  case LEV_NEWS_COMMENT+20 ... LEV_NEWS_COMMENT+24:
    user_id = ((struct lev_news_comment *) rec)->owner;
    break;
  case LEV_NEWS_PLACEDEL+20 ... LEV_NEWS_PLACEDEL+24:
  case LEV_NEWS_HIDEITEM+20 ... LEV_NEWS_HIDEITEM+24:
  case LEV_NEWS_SHOWITEM+20 ... LEV_NEWS_SHOWITEM+24:
  ...
  case LEV_BOOKMARK_INSERT + 20 ... LEV_BOOKMARK_INSERT + 24:
  case LEV_BOOKMARK_DELETE + 20 ... LEV_BOOKMARK_DELETE + 24:
    user_id = ((struct lev_user_generic *) rec)->a;
    break;
  case LEV_NEWS_RECOMMEND+0 ... LEV_NEWS_RECOMMEND+31:
    user_id = ((struct lev_news_recommend *) rec)->user_id;
    break;
  case LEV_NEWS_SET_RECOMMEND_RATE+0 ... LEV_NEWS_SET_RECOMMEND_RATE+31:
    user_id = copy_rem; /* always write */
    break;
  default:
