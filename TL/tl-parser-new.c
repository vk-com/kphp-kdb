...
struct tl_combinator_tree *tl_get_var_value (tree_var_value_t **T, struct tl_combinator_tree *var) {
  struct tl_var_value t = {.ptr = var, .val = 0, .num_val = 0};
  struct tl_var_value *r = tree_lookup_value_var_value (*T, t);
  return r ? r->val : 0;
}

int tl_get_var_value_num (tree_var_value_t **T, struct tl_combinator_tree *var) {
...
struct tl_program *tl_parse (struct tree *T) {
  ...
  int i;
  tl_program_cur = zmalloc (sizeof (*tl_program_cur));
  tl_add_type ("#", 1, 0, 0);
...
void write_tree (struct tl_combinator_tree *T, int extra, tree_var_value_t **v, int *last_var) {
  ...
  case type_list:
    ...
    break;
  case type_num_value:
    wint (schema_version >= 1 ? schema_version >= 2 ? TLS_NAT_CONST : TLS_TREE_NAT_CONST : -4);
    if (schema_version >= 2) {
      wint (T->type_flags);
    } else {
      wll (T->type_flags);
    ...
    break;
  case type_num:
    wint (schema_version >= 1 ? schema_version >= 2 ? TLS_NAT_VAR : TLS_TREE_NAT_VAR : -5);
    if (schema_version >= 2) {
      wint (T->type_flags);
    } else {
      wll (T->type_flags);
