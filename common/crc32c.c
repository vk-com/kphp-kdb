...
static unsigned crc32c_partial_old (const void *data, int len, unsigned crc) {
  const char *p = data;
  for (; len > 0; len--) {
...
int crc32c_check_and_repair (void *input, int l, unsigned *input_crc32c, int force_exit) {
  unsigned computed_crc32c = compute_crc32c (input, l);
  const unsigned crc32c_diff = computed_crc32c ^ (*input_crc32c);
  if (!crc32c_diff) {
    return 0;
  }
  int k = find_corrupted_bit (l, crc32c_diff);
  vkprintf (3, "find_corrupted_bit returns %d.\n", k);
  int r = repair_bit (input, l, k);
  vkprintf (3, "repair_bit returns %d.\n", r);
  if (!r) {
    assert (compute_crc32c (input, l) == *input_crc32c);
    if (force_exit) {
      kprintf ("%s successfully repair one bit in %d bytes block.\n", __func__, l);
    }
    return 1;
  }
  if (!(crc32c_diff & (crc32c_diff - 1))) { /* crc32c_diff is power of 2 */
    *input_crc32c = computed_crc32c;
    if (force_exit) {
      kprintf ("%s repair one bit in crc32 (%d bytes block).\n", __func__, l);
    }
    return 2;
  }
  assert (!force_exit);
